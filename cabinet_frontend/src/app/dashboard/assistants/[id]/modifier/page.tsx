...
export default function ModifierAssistantPage() {
    const router = useRouter();
    const params = useParams();
    const assistantId = Number(params.id);
    const [loading, setLoading] = useState(false);
    const [loadingData, setLoadingData] = useState(true);
    const [formData, setFormData] = useState<AssistantDTO>({
        nom: "",
        prenom: "",
        email: "",
        telephone: "",
    ...
    const handleChange = (field: keyof AssistantDTO, value: any) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            await assistantsApi.update(assistantId, formData);
    ...
    return (
        <div className="space-y-4">
            <div className="flex gap-4 items-center">
                ...
                <div>
                    <h1 className="text-4xl font-bold bg-gradient-to-r from-cyan-600 to-teal-600 bg-clip-text text-transparent">Modifier l'assistant</h1>
                    <p className="text-gray-600 mt-1">Mettez à jour les informations de l'assistant</p>
                </div>
            </div>
